%% World-Class Scientific Conference Poster Template
%% tikzposter class | Landscape A0 | 3-column layout
%% Designed for top-tier conferences (NeurIPS, ICML, CVPR, ACL, CHI, etc.)
%% Compile with: pdflatex poster.tex
%%
%% Design principles applied:
%%   - Landscape orientation (standard for top conferences)
%%   - Minimal text (~500 words), maximum visual impact
%%   - 40% whitespace, large readable fonts, high-contrast colors
...
\documentclass[25pt, a0paper, landscape, margin=15mm,
    innermargin=15mm, blockverticalspace=15mm, colspace=15mm]{tikzposter}
...
\definetitlestyle{ConferenceTitle}{
    ...
    \begin{scope}[line width=\titlelinewidth, rounded corners=\titleroundedcorners]
        ...
        \fill[color=amber] (\titleposleft,\titleposbottom) rectangle (\titleposright,\titleposbottom+0.4cm);
    \end{scope}
}
...
\defineblockstyle{ConferenceBlock}{
    ...
    titleinnersep=12mm, bodyinnersep=15mm
}{
...
\title{\parbox{0.85\linewidth}{\centering\textbf{%
    Efficient Neural Architecture Search via Gradient-Based\\[0.3em]
    Optimization with Adaptive Resource Allocation}}}
...
% CUSTOMIZATION: Uncomment and replace with your institution logos
% \titlegraphic{%
%     \includegraphics[height=7cm]{logo-left.png}%
%     \hfill%
%     \includegraphics[height=7cm]{logo-right.png}%
% }
...
% Middle column wider for the main results (visual focus)
\begin{columns}

    % ============================================================
    % COLUMN 1: Motivation + Methods (30%)
    % ============================================================
    \column{0.30}
    ...
    \block{Method: ENAS-Adaptive}{
        ...
        \begin{tikzpicture}[
            ...
            box/.style={rectangle, draw=navyblue, fill=steelblue!10,
                        text width=0.7\linewidth, text centered, rounded corners=5pt,
            ...
            % Feedback loop
            \draw[arrow, dashed, color=amber] (allocate.east) -- ++(2.5cm,0) |- (supernet.east)
    ...
    % ============================================================
    % COLUMN 2: Results -- the visual centerpiece (40%)
    % ============================================================
    \column{0.40}
    ...
    \block{Main Results}{
        ...
        \begin{tikzfigure}[Accuracy vs.\ search cost across NAS methods on CIFAR-10]
            \begin{tikzpicture}
                \begin{axis}[
                    ...
                    legend style={at={(0.02,0.98)}, anchor=north west, font=\large, draw=none, fill=white!80},
        ...
        % Results table -- the quantitative details
        \begin{center}
        {\large
        \begin{tabular}{lcccc}
            \toprule
        ...
        \begin{tikzfigure}[Search cost vs.\ accuracy trade-off. Circle size indicates model parameters (M).]
            \begin{tikzpicture}
                \begin{axis}[
                    ...
                    % Scatter points for each method (x=cost, y=accuracy)
    ...
    % ============================================================
    % COLUMN 3: Analysis + Conclusions (30%)
    % ============================================================
    \column{0.30}
    ...
    \block{Key Findings}{
        \begin{enumerate}[leftmargin=*, itemsep=0.8em]
            \item \textbf{Speed:} 73\% faster search than DARTS with higher accuracy
            \item \textbf{Transferability:} Architecture found on CIFAR-10 transfers well to ImageNet (+1.3\% over best baseline)
            \item \textbf{Adaptive allocation} is the key enabler --- reallocating from bottom 30\% of candidates yields 2.75$\times$ speedup
            \item \textbf{Robustness:} Consistent gains across 3 datasets, 5 search runs (std $<$ 0.1\%)
        \end{enumerate}
    }

    \block{Conclusions \& Future Work}{
        We present \textbf{ENAS-Adaptive}, a gradient-based NAS method with adaptive resource allocation that achieves state-of-the-art accuracy at a fraction of the search cost.

        \vspace{1.5em}
        \textbf{Future directions:}
        \begin{itemize}[leftmargin=*, itemsep=0.5em]
            \item Extend to detection and segmentation tasks
            \item Investigate hardware-aware search objectives
            \item Scale to larger search spaces (100K+ candidates)
        \end{itemize}
    }
    ...
    \block{References}{
        ...
        [4] Chu et al. (2021). FairNAS: Rethinking Evaluation Fairness. \textit{ICCV}.

        [5] Xie et al. (2019). SNAS: Stochastic NAS. \textit{ICLR}.
        }

        \vspace{1.5em}
        \textbf{Acknowledgments:} Supported by NSF Grant IIS-2024000 and Google Research Award.
    }
