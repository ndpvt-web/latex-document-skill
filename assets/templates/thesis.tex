...
\usepackage{fancyhdr}
\usepackage{titlesec}
\usepackage{emptypage}  % Blank pages between chapters have no headers
\usepackage{appendix}
...
\begin{itemize}[itemsep=0.5em]
    \item We propose [contribution 1] (Chapter~\ref{ch:method}).
    \item We demonstrate [contribution 2] (Chapter~\ref{ch:experiments}).
...
\begin{description}[itemsep=0.3em]
    \item[Chapter~\ref{ch:background}] reviews related work and background concepts.
    \item[Chapter~\ref{ch:method}] presents our proposed methodology.
    \item[Chapter~\ref{ch:experiments}] describes experimental evaluation.
    \item[Chapter~\ref{ch:conclusion}] concludes with a summary and future directions.
\end{description}
...
Use TikZ or \texttt{\\includegraphics} here
\vspace{2cm}}}
...
The complete procedure is summarized in Algorithm~\ref{alg:method}.

\begin{table}[H]
\centering
\caption{Algorithm: Proposed Method}
\label{alg:method}
\begin{tabular}{l}
\toprule
\textbf{Input:} Training data $\{(\mathbf{x}_i, \mathbf{y}_i)\}_{i=1}^N$, learning rate $\eta$ \\
\textbf{Output:} Trained parameters $\theta^*$ \\
\midrule
1: Initialize $\theta_0$ randomly \\
2: \textbf{for} $t = 1, 2, \ldots, T$ \textbf{do} \\
3: \quad Sample mini-batch $\mathcal{B} \subset \{1, \ldots, N\}$ \\
4: \quad Compute gradient $g_t = \nabla_\theta \mathcal{L}_\mathcal{B}(\theta_{t-1})$ \\
5: \quad Update $\theta_t = \theta_{t-1} - \eta \cdot g_t$ \\
6: \textbf{end for} \\
7: \textbf{return} $\theta_T$ \\
\bottomrule
\end{tabular}
\end{table}
...
Table~\ref{tab:datasets} summarizes the datasets used in our evaluation.
...
Dataset C & 200,000 & 20,000 & 40,000 \\
\bottomrule
\end{tabular}
\end{table}
...
Our results demonstrate that [finding 1]. We observe that [finding 2], which aligns with the theoretical analysis in Section~\ref{sec:motivation}. The performance gap is most pronounced on [dataset/condition], suggesting that [insight].
